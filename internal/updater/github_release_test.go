...
func TestGitHubReleaseSourceResolvesFormalReleaseToImmutableImage(t *testing.T) {
	...
	if resolved.Version != "v1.2.0" || resolved.Image != "dirextalk/message-server:v1.2.0" || resolved.Digest != "sha256:"+strings.Repeat("b", 64) {
...
func TestGitHubReleaseSourceRejectsUntrustedOrInconsistentRelease(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if _, err := NewGitHubReleaseSource(releaseHTTPClient(test.index, test.release, test.checksumBody)).Resolve(context.Background()); err == nil {
...
func TestGitHubReleaseSourceRedirectAllowlist(t *testing.T) {
	...
	allowed := []string{
		testIndexURL,
		"https://release-assets.githubusercontent.com/github-production-release-asset/123?token=bounded-by-github",
		"https://objects.githubusercontent.com/github-production-release-asset/123?token=bounded-by-github",
	}
	for _, candidate := range allowed {
		parsed, err := url.Parse(candidate)
		if err != nil || validateReleaseAssetRedirect(parsed, tag) != nil {
			t.Fatalf("trusted release redirect rejected: %s", candidate)
		}
	}
	denied := []string{
		"http://release-assets.githubusercontent.com/asset",
		"https://attacker.example/asset",
		"https://github.com/YingSuiAI/dirextalk-message-server/releases/download/v9.9.9/release-index.json",
		"https://attacker@github.com/YingSuiAI/dirextalk-message-server/releases/download/v1.2.0/release-index.json",
	}
	for _, candidate := range denied {
		parsed, err := url.Parse(candidate)
		if err == nil && validateReleaseAssetRedirect(parsed, tag) == nil {
			t.Fatalf("untrusted release redirect accepted: %s", candidate)
		}
	}
}
