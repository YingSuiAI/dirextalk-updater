...
`serve` 和 `trigger-discovery` 启动前会校验宿主机，非 Ubuntu 22.04 或 24.04
...
版本发现只接受 latest 正式 GitHub Release 中的 `release-index.json` 与
`release-index.json.sha256`。checksum 绑定完整 index，每个内嵌 manifest 还要
有独立摘要，每条升级边必须绑定精确的来源镜像摘要。优先直接升级边；否则到
latest 的路径必须唯一，并在创建任务前把完整有序链固化到 plan。
...
多跳升级会在每一跳分别轮换唯一备份、激活不可变镜像、执行迁移并通过完整健康
门禁，之后才进入下一跳。实际来源镜像摘要不在该边白名单时，会在目标变更前
停止；后续跳失败时只回滚到最近一个已验证健康的版本。
